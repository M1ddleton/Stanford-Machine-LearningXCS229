...
def main(is_semi_supervised, trial_num):
    ...
    for k in range(K):
        ...
        cov = np.cov(x_splits[k].transpose())
        mu.append(mean)
        sigma.append(cov)
    # *** END CODE HERE ***
...
def run_semi_supervised_em(x, x_tilde, z_tilde, w, phi, mu, sigma, max_iter=1000):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        for k in range(K):
            x_tilde_labeled = [y for x, y in zip(z_tilde == k, x_tilde) if x == True]
            x_tilde_labeled = np.array(x_tilde_labeled)
            for i in range(x_tilde_labeled.shape[0]):
                likelihood = np.exp(-0.5 * (np.dot(np.dot((x_tilde_labeled[i] - mu[k]).T, np.linalg.pinv(sigma[k])),
                                                   x_tilde_labeled[i] - mu[k])))
                likelihood = np.true_divide(likelihood, np.power(2.0 * np.pi, x_tilde_labeled.shape[1] / 2) * np.power(
                    np.linalg.det(sigma[k]), 0.5)).flatten()
        ...
        for k in range(K):
            ...
            sum_w_k = w[:, k].sum()
            x_tilde_labeled = np.array([y for x, y in zip(z_tilde == k, x_tilde) if x == True])
            phi[k] = np.true_divide(sum_w_k + sum_w_tilde_k, n + alpha * n_tilde)
            mu[k] = np.true_divide(np.dot(w[:, k], x) + alpha * x_tilde_labeled.sum(axis=0), sum_w_k +sum_w_tilde_k)
            ...
            for i in range(x_tilde_labeled.shape[0]):
                x_mu = np.reshape(x_tilde_labeled[i] - mu[k], (mu[k].shape[0], 1))
