...
class Test_2b(GradedTestCase):
  def setUp(self):
    ...
    self.train_data, self.train_labels = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.read_data('images_train.csv', 'labels_train.csv', max_rows=N_ROWS))
    # to minimize floating point rounding error during the testing, use the normalized value instead of the raw value
    self.train_data_norm = self.train_data/max(self.train_data[0])
    self.train_labels = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.one_hot_labels(self.train_labels))
  ...
  def test_0(self):
    ...
    student_grad = submission.backward_prop_regularized(data=self.train_data_norm, labels=self.train_labels, params=params,
                                                     forward_prop_func=solution_forward_prop, reg=0.0001)
    solution_grad = solution_backward_prop_regularized(data=self.train_data_norm, labels=self.train_labels, params=params,
                                                       forward_prop_func=solution_forward_prop, reg=0.0001)
    results_match = True
  ...
  def test_1(self):
    ...
    for _ in range(10):
      # updates params
      submission.gradient_descent_epoch(self.train_data_norm, self.train_labels, learning_rate=0.1, batch_size=5,
                                     params=student_params,
                                     forward_prop_func=solution_forward_prop,
                                     backward_prop_func=lambda a, b, c, d: submission.backward_prop_regularized(a, b,
                                                                                                             ...
                                                                                                             d,
                                                                                                             reg=0.0001))
      solution_gradient_descent_epoch(self.train_data_norm, self.train_labels, learning_rate=0.1, batch_size=5,
                                      params=solution_params,
                                      forward_prop_func=solution_forward_prop,
