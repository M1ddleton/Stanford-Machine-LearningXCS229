...
class Test_3a(GradedTestCase):
  ...
  def test_0(self):
    """3a-0-hidden: spam (`get_words` check)"""
  ...
  @graded(is_hidden=True)
  def test_1(self):
    """3a-1-hidden: spam (`create_dictionary` check)"""
    ...
    student_set = set(student_result.keys())
    solution_set = set(solution_result.keys())
    self.assertTrue(student_set == solution_set)

  @graded(is_hidden=True)
  def test_2(self):
    """3a-2-hidden: spam (`transform_text` output shape check)"""
    solution_create_dictionary = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.create_dictionary)
    solution_transform_text = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.transform_text)

    solution_dictionary = solution_create_dictionary(self.train_messages)
    student_dictionary = submission.create_dictionary(messages=self.train_messages)
    student_result = submission.transform_text(messages=self.train_messages, word_dictionary=student_dictionary)
    solution_result = solution_transform_text(messages=self.train_messages, word_dictionary=solution_dictionary)
    self.assertTrue(student_result.shape == solution_result.shape)

  @graded(is_hidden=True)
  def test_3(self):
    """3a-3-hidden: spam (Size of dictionary check)"""
    solution_create_dictionary = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.create_dictionary)
    student_result = submission.create_dictionary(messages=self.train_messages)
    solution_result = solution_create_dictionary(messages=self.train_messages)
  ...
  @graded()
  def test_4(self):
    """3a-4-basic:  Create the spam_dictionary and spam_sample_train_matrix files"""
...
class Test_3b(GradedTestCase):
  ...
  @graded(is_hidden=True)
  def test_0(self):
    """3b-0-hidden: spam (`fit_naive_bayes_model` accuracy check [>90%])"""
    solution_create_dictionary = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.create_dictionary)
    solution_transform_text = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.transform_text)
    ...
    for model_param in solution_model:
      self.assertTrue(np.allclose(solution_model[model_param], student_model[model_param], atol=1e-1), f"Model parameter {model_param} does not match solution")
    ...
    self.assertTrue(student_model_accuracy * 100 >= 90)

  @graded()
  def test_1(self):
    """3b-1-basic:  compute naive bayes prediction accuracy and then save resulting predictions"""
    train_messages, train_labels = util.load_spam_dataset('spam_train.tsv')
    val_messages, val_labels = util.load_spam_dataset('spam_val.tsv')
    test_messages, test_labels = util.load_spam_dataset('spam_test.tsv')
...
class Test_3c(GradedTestCase):
  ...
  @graded(is_hidden=True)
  def test_0(self):
    """3c-0-hidden: spam (`get_top_five_naive_bayes_words`)"""
    solution_create_dictionary = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.create_dictionary)
    solution_transform_text = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.transform_text)
    solution_fit_naive_bayes_model = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.fit_naive_bayes_model)
  ...
  @graded()
  def test_1(self):
    """3c-1-basic:  Calculate the top five most indicative words"""
    train_messages, train_labels = util.load_spam_dataset('spam_train.tsv')
    dictionary = submission.create_dictionary(train_messages)
    train_matrix = submission.transform_text(train_messages, dictionary)
    naive_bayes_model = submission.fit_naive_bayes_model(train_matrix, train_labels)
...
class Test_3d(GradedTestCase):
  ...
  def test_0(self):
    """3d-0-hidden: svm (`compute_best_svm_radius`)"""
    solution_create_dictionary = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.create_dictionary)
    solution_transform_text = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.transform_text)
  ...
  def test_1(self):
    """3d-1-hidden: svm (Accuracy > 95% on a sample test set trained on SVM with `optimal_radius` computed using `compute_best_svm_radius`)"""
    solution_create_dictionary = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.create_dictionary)
    solution_transform_text = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.transform_text)
